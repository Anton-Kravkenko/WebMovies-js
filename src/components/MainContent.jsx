...
import { FilmContext } from "../components/App";
import "swiper/css";
function MainBlock() {
  const {
    filteringAll,
    ...
    <div className="Main__wraper_Content">
      ...
      <div className="MainBaners">
        {getMultipleRandom(Film, 1).map((items) => (
          <div
            className="Film_Map_content"
            style={{ backgroundImage: `url(${items.MovieAvatar})` }}
          >
            <Link
            ...
            >
              <div className="Text_content_baner">
                ...
                <h4 className="Description_text">
                  {items.Description.slice(0, 135)}...
                </h4>
                <p className="info_content_text">
                  {items.YearsOld} | {items.genre} | {items.Duration}
                </p>
              </div>
            </Link>
          </div>
